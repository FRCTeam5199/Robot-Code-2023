...
public class DriveManagerSwerve extends AbstractDriveManager {
    ...
    public void init() {
        ...
        setKin();
        limeLightPid = new PIDController(robotSettings.limeLightPid.P, robotSettings.limeLightPid.I,
                robotSettings.limeLightPid.D);
    ...
    public SubsystemStatus getSubsystemStatus() {
        if (driverFR.driver.isFailed() || driverBR.driver.isFailed() || driverFL.driver.isFailed()
                || driverBL.driver.isFailed() || driverFR.steering.isFailed() || driverBR.steering.isFailed()
                || driverFL.steering.isFailed() || driverBL.steering.isFailed())
    ...
    public void initTeleop() {
        ...
        resetSteeringEncoders();
        setBrake(true);
        // setSteeringContinuous(FLcoder.getAbsolutePosition(),
        // FRcoder.getAbsolutePosition(),BLcoder.getAbsolutePosition(),BRcoder.getAbsolutePosition());
    ...
    private void driveSwerve() {
        ...
        } else if (Math.abs(xbox.get(DefaultControllerEnums.XboxAxes.RIGHT_JOY_X)) <= 0.15
                && Math.abs(xbox.get(DefaultControllerEnums.XboxAxes.LEFT_JOY_X)) <= 0.15
                && Math.abs(xbox.get(DefaultControllerEnums.XboxAxes.LEFT_JOY_Y)) <= 0.15) {
            rotation = 0;
            // startHeading = guidance.imu.relativeYaw();
    ...
    private void setSteeringContinuous(double FL, double FR, double BL, double BR) {
        ...
        // System.out.println("Absolute Position/F current positiion FL: " +
        // FLcoder.getAbsolutePosition());
        // System.out.println("Absolute Position/ current positiion FR: " +
        // FRcoder.getAbsolutePosition());
        // System.out.println("Absolute Position/ current positiion BL: " +
        // BLcoder.getAbsolutePosition());
        // System.out.println("Absolute Position/ current positiion BR: " +
        // BRcoder.getAbsolutePosition());
        // System.out.println("turning speed/pid should be: " +
        // FLpid.calculate(FLcoder.getAbsolutePosition()));
        ...
        // driverFL.getState(), driverFR.getState(), driverBL.getState(),
        // driverBR.getState()
    ...
    private void setDrive(double FL, double FR, double BL, double BR) {
        ...
        double gearRatio = 1;// robotSettings.SWERVE_SDS_DRIVE_BASE.getDriveReduction() *
                             // robotSettings.SWERVE_SDS_DRIVE_BASE.getWheelDiameter();
        double voltageMult = 95 / 371.0; // 127.4/371.0 is full speed
        // System.out.println(adjustedDriveVoltage((FPS_FR) * gearRatio *
        // robotSettings.DRIVE_SCALE, voltageMult));
        driverFR.driver
                .moveAtVoltage(-adjustedDriveVoltage((FPS_FR) * gearRatio * robotSettings.DRIVE_SCALE, voltageMult));
        driverFL.driver
                .moveAtVoltage(-adjustedDriveVoltage((FPS_FL) * gearRatio * robotSettings.DRIVE_SCALE, voltageMult));
        driverBR.driver
                .moveAtVoltage(-adjustedDriveVoltage((FPS_BR) * gearRatio * robotSettings.DRIVE_SCALE, voltageMult));
        driverBL.driver
                .moveAtVoltage(-adjustedDriveVoltage((FPS_BL) * gearRatio * robotSettings.DRIVE_SCALE, voltageMult));
    }
    ...
    public void driveMPS(double xMeters, double yMeters, double rotation) { // after here
        ...
        UserInterface.smartDashboardPutNumber("tot magitude climb",
                Math.sqrt((Math.pow(guidance.imu.relativeRoll(), 2) + Math.pow(guidance.imu.relativePitch(), 2))));
        ChassisSpeeds speeds;
        // System.out.println("xmeters: + " + xMeters + "ymeters: " + yMeters +
        // "rotation + " + rotation);
        ...
        if (useLocalOrientation()) {
            ...
            speeds = ChassisSpeeds.fromFieldRelativeSpeeds(xMeters, yMeters, rotation, Rotation2d
                    .fromDegrees(-guidance.swerveRobotPose.getEstimatedPosition().getRotation().getDegrees()));
        }
    ...
    public void driveWithChassisSpeeds(ChassisSpeeds speeds) {
        ...
        SwerveModuleState frontLeft = moduleStates[0], frontRight = moduleStates[1], backLeft = moduleStates[2],
        ...
        // if(xbox.get(DefaultControllerEnums.XboxAxes.RIGHT_JOY_X) == 0 &&
        // xbox.get(DefaultControllerEnums.XboxAxes.LEFT_JOY_X) == 0 &&
        // xbox.get(DefaultControllerEnums.XboxAxes.LEFT_JOY_Y) == 0) {
        // setDrive(0, 0, 0, 0); // before here
        // }else {
        setSteeringContinuous(frontLeft.angle.getDegrees(), frontRight.angle.getDegrees(), backLeft.angle.getDegrees(),
                backRight.angle.getDegrees()); // <-- maybe here

        setDrive(frontLeft.speedMetersPerSecond, frontRight.speedMetersPerSecond, backLeft.speedMetersPerSecond,
                backRight.speedMetersPerSecond);
        // }
        if (DEBUG) {
            // System.out.printf("%4f %4f %4f %4f \n", frontLeft.speedMetersPerSecond,
            // frontRight.speedMetersPerSecond, backLeft.speedMetersPerSecond,
            // backRight.speedMetersPerSecond);
        }
    }
     ...
     * @deprecated (For now, dont use this since the PID in the motors arent
     *             continuous)
     */
    @Deprecated
    ...
    private void setKin() {
        Translation2d FLPos = Objects.requireNonNullElseGet(frontLeftLocation,
                () -> frontLeftLocation = new Translation2d(-trackLength / 2 / 39.3701, trackWidth / 2 / 39.3701));
        Translation2d FRPos = Objects.requireNonNullElseGet(frontRightLocation,
                () -> frontRightLocation = new Translation2d(-trackLength / 2 / 39.3701, -trackWidth / 2 / 39.3701));
        Translation2d BLPos = Objects.requireNonNullElseGet(backLeftLocation,
                () -> backLeftLocation = new Translation2d(trackLength / 2 / 39.3701, trackWidth / 2 / 39.3701));
        Translation2d BRPos = Objects.requireNonNullElseGet(backRightLocation,
                () -> backRightLocation = new Translation2d(trackLength / 2 / 39.3701, -trackWidth / 2 / 39.3701));
        kinematics = Objects.requireNonNullElseGet(kinematics,
    ...
    public SwerveModulePosition[] getModulePosition() {
        return new SwerveModulePosition[] {
                new SwerveModulePosition(driverFL.driver.getRotations(),
                        Rotation2d.fromDegrees(FLcoder.getAbsolutePosition())),
                new SwerveModulePosition(driverFR.driver.getRotations(),
                        Rotation2d.fromDegrees(FLcoder.getAbsolutePosition())),
                new SwerveModulePosition(driverBL.driver.getRotations(),
                        Rotation2d.fromDegrees(FLcoder.getAbsolutePosition())),
                new SwerveModulePosition(driverBR.driver.getRotations(),
                        Rotation2d.fromDegrees(FLcoder.getAbsolutePosition()))
        };
    }
    ...
    private void createDriveMotors() throws InitializationFailureException {
        driverFR = new SwerveMotorController(robotSettings.SWERVE_DRIVE_FR, robotSettings.DRIVE_MOTOR_TYPE,
                robotSettings.SWERVE_TURN_FR, robotSettings.DRIVE_MOTOR_TYPE);
        driverBR = new SwerveMotorController(robotSettings.SWERVE_DRIVE_BR, robotSettings.DRIVE_MOTOR_TYPE,
                robotSettings.SWERVE_TURN_BR, robotSettings.DRIVE_MOTOR_TYPE);
        driverBL = new SwerveMotorController(robotSettings.SWERVE_DRIVE_BL, robotSettings.DRIVE_MOTOR_TYPE,
                robotSettings.SWERVE_TURN_BL, robotSettings.DRIVE_MOTOR_TYPE);
        driverFL = new SwerveMotorController(robotSettings.SWERVE_DRIVE_FL, robotSettings.DRIVE_MOTOR_TYPE,
                robotSettings.SWERVE_TURN_FL, robotSettings.DRIVE_MOTOR_TYPE);

        // rpm <=> rps <=> gearing <=> wheel circumference
        driverFR.driver.setRealFactorFromMotorRPM(
                robotSettings.DRIVE_GEARING * (robotSettings.WHEEL_DIAMETER * Math.PI / 12), 1 / 60D);
        driverFL.driver.setRealFactorFromMotorRPM(
                robotSettings.DRIVE_GEARING * (robotSettings.WHEEL_DIAMETER * Math.PI / 12), 1 / 60D);
        driverBR.driver.setRealFactorFromMotorRPM(
                robotSettings.DRIVE_GEARING * (robotSettings.WHEEL_DIAMETER * Math.PI / 12), 1 / 60D);
        driverBL.driver.setRealFactorFromMotorRPM(
                robotSettings.DRIVE_GEARING * (robotSettings.WHEEL_DIAMETER * Math.PI / 12), 1 / 60D);
    ...
    public void setCANCoder() {
        ...
        FLcoder.configMagnetOffset(/*245*/305);
        FRcoder.configMagnetOffset(/*135*/225);
        BLcoder.configMagnetOffset(/*275*/275);
        BRcoder.configMagnetOffset(/*195*/165);
    }
    ...
    public boolean driveWheetRot(double move) {
        ...
        // System.out.println("original motor rotation: " + currentMotorRot + "motorRot
        // = " + motorRot + "how far motor has moved: " +
        // driverFR.driver.getRotations());
    ...
    public boolean turnDegree(int degrees) {
        ...
        if (orgDeg + degrees >= guidance.imu.relativeYaw() && degrees > 0) {
            ...
            // System.out.println("The original degree was: " + orgDeg + "The Current Degree
            // was: " + guidance.imu.relativeYaw());
            return false;
    ...
    public boolean leveling() {
        ...
        double totalMagnetude = Math
                .sqrt((Math.pow(guidance.imu.relativeRoll(), 2) + Math.pow(guidance.imu.relativePitch(), 2)));
    ...
    public boolean driveForwardWithAngle() {
        ...
        if (DriverStation.getAlliance() == DriverStation.Alliance.Blue) {
            rotation = UtilFunctions.mathematicalMod(
                    (180 - guidance.swerveRobotPose.getEstimatedPosition().getRotation().getDegrees()) + 180, 360)
                    - 180;
        } else {
            rotation = UtilFunctions.mathematicalMod(
                    (0 - guidance.swerveRobotPose.getEstimatedPosition().getRotation().getDegrees()) + 180, 360) - 180;
        }
