...
public class Point {
    ...
    /**
     * Use this to determine if two points are within a tolerance while keeping code
     * clean
     *
     * @param distance   max tolerance between this and another point
     * @param otherPoint the other point to compare distance to
     * @return whether the distance between this and the other point is less than
    ...
    public boolean isWithin(double distance, Point otherPoint) {
        ...
        // System.out.println(Math.abs(-Y - otherPoint.Y) + " " + Math.abs(X -
        // otherPoint.X));
    ...
    public boolean isWithinEllipse(double distx, double disty, Point otherPoint) {
        return Math.pow(otherPoint.X - X, 2) / Math.pow(distx, 2)
                + Math.pow(otherPoint.Y - Y, 2) / Math.pow(disty, 2) <= 1;
    }

    /**
     * its just like subtracting numbers but all wrapped for maximum onelining
     * capabilities
     *
     * @param other the point to subtract from this point
     * @return these coordinates minus the provided coordinates wrapped in a new
     *         point
     */
    ...
    /**
     * This method means you can just System.out.println(point) without having to
     * format it Result format "(x, y) "
     *
