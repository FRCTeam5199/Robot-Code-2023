...
public class ManipulationManager implements ISubsystem {
    ...
    public boolean checkArmPassover(){
        if (-70 + slippageOffSet >= Robot.arm.armRotationController.getRotations() && Robot.arm.armRotationController.getRotations() >= -160+  slippageOffSet){
            Robot.elevator.moveElevator(-44);
            return true;
        }else if(-60 + slippageOffSet >= Robot.arm.armRotationController.getRotations()  && Robot.arm.armRotationController.getRotations() >= -178+ slippageOffSet){
    ...
    public boolean checkArmCollision(){
        if(Robot.arm.armRotationController.getRotations() >= 10 + slippageOffSet || Robot.arm.armRotationController.getRotations() <= -300 + slippageOffSet){
           return true;
        }
        return false;
    }
    ...
    public boolean goTo(double elevator, double arm){
        ...
        if(Math.abs(elevator - Robot.elevator.elevate.getRotations()) <= .2 && Math.abs(arm - Robot.arm.armRotationController.getRotations()) <= .2) {
            return true;
        }
